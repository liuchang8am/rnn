...
function AbstractRecurrent:updateGradInput(input, gradOutput)      
   if self.onlineBackward then
      ...
      if self.copyGradOutputs then
         ...
         self.gradOutputs[self.updateGradInputStep-1] = self.gradOutputs[self.updateGradInputStep-1] or utils_recursiveNew(gradOutput)
...
function utils_recursiveNew(t2)
   if torch.type(t2) == 'table' then
      local t1 = {}
      for key,_ in pairs(t2) do
         t1[key] = utils_recursiveNew(t2[key])
      end
      return t1
